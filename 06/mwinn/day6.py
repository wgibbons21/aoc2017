...
class register:
    ...
    def __init__(self, line):
        ## Load string passed in from argument into the current register
        self.reg = line
        self.history = []
    ...
    def redistribute(self):
        sys.stdout.write("Stage {}: redistributing...\r".format(len(self.history)))
        sys.stdout.flush()
        i = self.reg.index(max(self.reg))       # i is the original index of the max value
        mx = self.reg[i]                         # n is the actual max value at index i
        self.reg[i] = 0                         # zero out the index

        x = (i + 1) % self.size                 # increment the moving index (w/wraparound)
                                                # so that the for loop starts at the next cell

        for j in range(mx):                     # loop the moving index (around) until arriving at the original index
            self.reg[x] += 1                    # add the increment value to the existing value
            x = (x + 1) % self.size             # increment the moving index (w/wraparound)

    def compare(self):
        if self.reg in self.history:
            print('Found duplicate!')
            self.seen = True
        else:
            self.history.append(list(self.reg))
...
def main():
    ...
    print("Part 1 result: {}\t{}").format(len(reg.history)+1, reg.reg)
    
    print("Part 2 result: {}\t{}").format(len(reg.history)+1, reg.reg)
