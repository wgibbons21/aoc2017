...
#       moves past the end of the list, it wraps around to the front. Lengths larger than the size of the list are
#       invalid.
#
...
#  your puzzle input. Once this process is complete, what is the result of multiplying the first two numbers
#  in the list?
#
# --- Part Two ---
#
...
#  previous example was your first round, you would start your second round with the same length sequence
...
#############################################################################
# APPROACH (using python 3 for this one)
#   Create a customcrypto class
#   Build the following variables
#       A list (from 0...255)
#       A counter variable
#       A length (populated for each item in the list read in from the input file)
#       A current position counter
#   Build the following functions
#       Extract the subset of length from the list (mod to wrap around)
#       Reverse the extracted list
#       Increment the counter
#       Adjust the counter (mod to wrap around)
#############################################################################
class knothash:
    """Implements the elves knot-hash algorithm"""

    def __init__(self):
        """Initializes the data."""
        self.numlist = list(range(0, 256))
        self.wrap = len(self.numlist)
        self.skip = 0
        self.pos = 0

    def revsublist(self, l):
        p = self.pos
        print('\t(L{} @ p{})SUB: {}'.format(len(l), p, l))
        l = l[::-1]
        print('\t\t\t\tREV: {}'.format(l))
        for item in l:
            self.numlist[p] = item
    ...
    def sublist(self, size):
        return [self.numlist[i % self.wrap] for i in range(self.pos, self.pos + size)]

    def movepos(self, n):
        self.pos = (self.pos + n + self.skip) % self.wrap
...
def main():
    ...
    lstLine = list(map(int, lstLine))       # in python 3, the map function needs to be cast as a list
    f.close()

    hash = knothash()

    print('Initial list:\t\t{}'.format(hash.showlist()))
    print('input: {}'.format(lstLine))
    print('---------------------------------------------')

    for l in lstLine:
        print('Step {}, length {}'.format(hash.skip, l))
        r = (hash.sublist(l))
        hash.revsublist(r)
        hash.movepos(l)
        print('\n\t\t\t\t\t{}'.format(list(range(0, 255))))
        print('\tresult:\t\t\t{}'.format(hash.showlist()))
        print('------------------------------------------------------------')

    result = hash.showlist()
    print(len(result))
    print('\nPart 1 result: first {}, second {}, prod {}\n'.format(result[0], result[1], result[0] * result[1]))
