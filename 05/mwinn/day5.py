...
# (1) 3  0  1  -3  - jump with offset 0 (that is, don't jump at all). Fortunately, the instruction is incremented to 1.
...
def parseFile(fname):
    ...
    for line in f:      # We need to read the whole file into one list so that it can be navigated by index
        lstLine.append(int(line))

    while i < len(lstLine):

        n = lstLine[i]  # Jumps are relative: -1 moves to the previous instruction, and 2 skips the next one
        lstLine[i] += 1
        i += n
...
def parseFile2(fname):
    ...
    while i < len(lstLine):
        ...
        if n >= 3:  # If offset was three or more, instead decrease it by 1. Otherwise, increase it by 1 as before.
            lstLine[i] -= 1
        else:
            lstLine[i] += 1
